...
class _ProfileTestState extends State<ProfileTest> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(),
      body: BlocConsumer<ProcessContactBloc,ProcessContactState>(builder: (context,state){
                      ...
                      child: Row(
                        children: [
                              ...
                              :state.contactModel.status == '1' ?
                               state.contactModel.senderId == widget.uid ? Text("Hủy kết bạn")
                              : Text('Xac nhan kết bạn')
                              : Text("Thêm bạn bè"),
                          state.contactModel.status == '0'
                              ? Icon(Icons.people_alt)
                              :state.contactModel.status == '1' ?
                          state.contactModel.senderId == widget.uid ? Icon(Icons.person_add_disabled)
                              : Icon(Icons.check)
