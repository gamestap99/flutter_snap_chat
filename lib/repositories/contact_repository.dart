...
class ApiContactRepository implements ContactRepository {
  ...
  Future<String> getContactId(String uid, String peerId) async {
    ...
    if (snapshot.docs.length > 0) {
      snapshot.docs.forEach((element) {
        if (element['senderId'] == uid && element['receiverId'] == peerId || element['senderId'] == peerId && element['receiverId'] == uid) {
          // setState(() {
  ...
  @override
  Future<void> acpectContact(String contactId) {
    FirebaseFirestore.instance.collection('contacts').doc(contactId).update({'status': "0"}).then((value) {}).catchError((ex) {
          throw Exception('acpectContact contact that bai');
        });
  }

  @override
  Future<void> addContactById(String contactId) {
    FirebaseFirestore.instance.collection('contacts').doc(contactId).update({'status': "1"}).then((value) {}).catchError((ex) {
          throw Exception('addContactById contact that bai');
        });
  }
  ...
  Future<void> deleteContact(String contactId) {
    FirebaseFirestore.instance.collection('contacts').doc(contactId).update({'status': "-1"}).then((value) {}).catchError((ex) {
          throw Exception('Delete contact that bai');
        });
  }
  ...
  Future<List<UserModel>> searchUsers(String name) async {
    ...
    var usersContain = users.where((element) => !element.name.toLowerCase().startsWith(name.toLowerCase()) && element.name.toLowerCase().contains(name.toLowerCase())).toList();
